...
export const getSuggestedUsers = async () => {
  ...
  try {
    ...
    const usersFollowedMyMe = await User.findById(userId).select("following");
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    if ((!newPassword && currentPassword) || (!currentPassword && newPassword)) {
      return res.status(404).json({ message: 'Please enter your current password and/or a new password.' });
    }
